...
SWEP.Purpose				= ""
SWEP.Instructions			= ""
SWEP.PrintName				= "Bullet Bill SANDBOX"
SWEP.Slot					= 7
SWEP.SlotPos				= 7
SWEP.DrawAmmo				= false
...
SWEP.Primary.ClipSize			= -1			-- Change those 3 Variables to whatever you like
SWEP.Primary.DefaultClip		= -1				-- if you want to shoot more than 1 rocket!
...
function SWEP:PrimaryAttack()
       ...
       // self.Weapon:SetNextPrimaryFire( CurTime() + 0.2 )
        ...
        if ( SERVER ) then
                ...
                grenade = ents.Create( "bullet_bill_small" )                        
                grenade:SetPos( self.Owner:GetShootPos() + (self.Owner:GetAimVector()*60))
                grenade.Creator = self.Owner
                grenade.FlyAngle = self.Owner:GetAimVector():Angle()
                grenade:Spawn()
                grenade.Speed = 350
                //grenade.Tracking = tracking
                local phys = grenade:GetPhysicsObject()
                if (phys:IsValid()) then
                        phys:SetVelocity( self.Owner:GetAimVector() * grenade.Speed)
                end
...
function SWEP:SecondaryAttack()
        ...
        if ( SERVER ) then
                ...
                local grenade = ents.Create( "bullet_bill_big_sandbox" )                        
                grenade:SetPos( self.Owner:GetShootPos() + (self.Owner:GetAimVector()*60))
                grenade.Creator = self.Owner
                grenade.FlyAngle = self.Owner:GetAimVector():Angle()
                grenade:Spawn()
